...
#include "valve/interfaces/imemalloc.h"
#include <stdint.h>

// @source: master/public/tier1/utlfixedmemory.h

#define CS_ASSERT(EXPRESSION) static_cast<void>(!!(EXPRESSION) || (__debugbreak(), 0))
...
public:
	...
	CUtlFixedMemory& operator=( CUtlFixedMemory&& ) = delete;		
	CUtlFixedMemory& operator=( const CUtlFixedMemory& ) = delete;
	...
	[[nodiscard]] static std::uintptr_t InvalidIndex( )
	{
		return 0;
	}
	...
	void Grow( const int nCount = 1 )
	{
		...
		BlockHeader_t* pNewBlockHeader = static_cast< BlockHeader_t* >( Interfaces::MemAlloc->Alloc( sizeof( BlockHeader_t ) + nBlockSize * sizeof( T ) ) );
	...
	void Purge( )
	{
		...
		for ( BlockHeader_t* pBlockHeader = pBlocks; pBlockHeader != nullptr;)
		{
			...
			pBlockHeader = pBlockHeader->pNext;
			Interfaces::MemAlloc->Free( pFree );
		}
