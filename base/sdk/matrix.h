...
struct Matrix3x4_t
{
	...
	Vector Transform( const Vector& vector ) const {
		return {
			...
			vector.Dot( arrData[ 1 ] ) + arrData[ 1 ][ 3 ],
			vector.Dot( arrData[ 2 ] ) + arrData[ 2 ][ 3 ]
		};
	}
...
struct ViewMatrix_t {
	...
	ViewMatrix_t( const Matrix3x4_t& matFrom, const Vector4D& vecAdditionalRow = {} )
	{
	...
	[[nodiscard]] Vector4D GetRow( const int nIndex ) const
	{
		return { arrData[ nIndex ][ 0 ], arrData[ nIndex ][ 1 ], arrData[ nIndex ][ 2 ], arrData[ nIndex ][ 3 ] };
	}

	[[nodiscard]] Vector4D GetColumn( const int nIndex ) const
	{
