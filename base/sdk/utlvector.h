...
public:
	...
	{
		ASSERT( &vecOther != this ); // self-assignment isn't allowed
	...
	{
		ASSERT( IsValidIndex( nIndex ) ); // given index is out of range
	...
	void CopyFromArray( const T* pArraySource, int nArraySize )
	{
		...
		ASSERT( memory.Base( ) == nullptr || pArraySource == nullptr || begin( ) >= ( pArraySource + nArraySize ) || pArraySource >= end( ) );
	...
	{
		ASSERT( IsValidIndex( nElement ) || nSize == 0 || nShift == 0 );

		if ( const int nToMove = nSize - nElement - nShift; nToMove > 0 && nShift > 0 )
			std::memmove( &Element( nElement + nShift ), &Element( nElement ), nToMove * sizeof( T ) );
	}
	...
	void ShiftElementsLeft( const int nElement, const int nShift = 1 )
	{
		...
		if ( const int nToMove = nSize - nElement - nShift; nToMove > 0 && nShift > 0 )
			std::memmove( &Element( nElement ), &Element( nElement + nShift ), nToMove * sizeof( T ) );
	}
	...
	{
		// can't insert something that's in the list. reallocation may hose us
		ASSERT( memory.Base( ) == nullptr || &source < begin( ) || &source >= end( ) );
	...
	int InsertBefore( const int nElement )
	{
		...
		ASSERT( nElement == nSize || IsValidIndex( nElement ) );
