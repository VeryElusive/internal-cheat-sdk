...
void CMenuItem::Checkbox( ) {
	...
	const auto hovered{ Input::Hovered( Menu::m_vecDrawPos, size + Vector2D{ Render::GetTextSize( m_szName, 13, Render::Fonts.Menu ).x + ITEM_HEIGHT * 2, 0 } ) };
	...
	Render::Text( Menu::m_vecDrawPos + Vector2D( ITEM_HEIGHT * 2, 0 ), m_szName, Color( 255, 255, 255 ), 0, 13, Render::Fonts.Menu );
